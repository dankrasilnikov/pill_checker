...
def supabase_login_view(
    ...
    try:
        ...
        return RedirectResponse(
            url="/login?error=Profile does not exist.", status_code=status.HTTP_404_NOT_FOUND
        )
    except Exception:
        logger.exception("Supabase login error for user: %s", email)
        return RedirectResponse(
            url="/login?error=Supabase login error for user", status_code=status.HTTP_403_FORBIDDEN
        )
...
async def update_profile(profile_updates: ProfileUpdateForm, user=Depends(supabase_login_required)):
    try:
